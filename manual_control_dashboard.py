...
def main():
    try:
        ...
        vehicle_transform = carla.Transform(carla.Location(x=100, y=100, z=0), carla.Rotation(yaw=0))
        ...
        while True:
            ...
            velocity = carla.vehicle.get_velocity()
            speed = 3.6 * math.sqrt(velocity.x**2 + velocity.y**2) # Converting to km/h
    
            # Retrieve steering angle (in radians)
            steering = carla.vehicle.get_control().steer
    
            # Display speed and steering on the dashboard
            font = pygame.font.Font(None, 36)
            speed_text= font.render(f"Speed: {speed:.2f} km/h", True, (255, 255, 255))
            steering_text = font.render(f"Steering: {steering:.2f} radians", True, (255, 255, 255))
    
            display.blit(speed_text, (10, 10))
            display.blit(steering_text, (10, 50))
    finally:
        pygame.quit()
